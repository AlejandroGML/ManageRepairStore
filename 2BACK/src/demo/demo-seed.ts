...
/**
 * Demo seed body — 100% synthetic data for the Manage Repair Store portfolio demo.
 * Aligned with docs/manage-repair-store.html (prototype): same users, products,
 * locations, orders and activity feed.
 *
 * Idempotent: truncates all demo tables and re-inserts from scratch, inside a
 * single transaction (readers never see a half-seeded state).
 *
 * Used by the CLI (pnpm run seed) and by POST /demo/reset (demo ephemerality).
 */
